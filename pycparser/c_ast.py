...
class Node(object):
    ...
    def reConstruct(self):
        if  (type(self) is For) or type (self) is DoWhile or type(self) is While:
            #print "\nLoop\n"
            self.setChildren(cond= self.cond,stmt= self.stmt)

        for idx, (child_name, child) in enumerate(self.children()):
            child.reConstruct()
        # set children for current node current.children = children
        ...
        if self.attr_names:
            if attrnames:
                ...
                if len(self.children())==0:
                     if len(vlist)>0:
                         leafname =''.join('%s' % v for v in vlist[0])
                         buf.write(leafname)
    ...
    def traverse2paragraph(self, buf=None, offset=0, attrnames=False, nodenames=False, showcoord=False, _my_node_name=None):
        # buf.write('(')
        #lead = '' * offset
        #buf.write(lead + self.__class__.__name__+ '')
        ...
        for (parent_name1, parent1) in self.children(): #root
            #buf.write('\n'+ parent_name1 +'\n')
            #firstnode = False
            # for (parent_name2, parent2) in parent1.children(): #root
                #buf.write('\n')
                for (child_name, child) in parent1.children():
                    # if not firstnode:
                    buf.write('<sssss>')
                    firstnode = False
                    child.traverse(
                    ...
                    _my_node_name=child_name)
                #buf.write(')')
    def traverse(self, buf= None, offset=0, attrnames=False, nodenames=False, showcoord=False, _my_node_name=None):
        # buf.write('(')
        lead = '' * offset
        buf.write(lead + self.__class__.__name__+ '')
        # if self.attr_names:
    ...
    def exporttofile(self, offset=0, attrnames=False, nodenames=False, dump = True, _my_node_name=None, f = None, delimitor ='(' ):
        if delimitor =='(':
            d1 ='('
            d2 = ')'
        else:
        ...
        if len(self.children())==0: #leaf node
             if dump:
                 ...
                 # else:
                    ...
                    if nodename =="Return" \
                            or nodename =="Break" \
                            or nodename =="Continue"\
                            or nodename=="Goto":
                        leafname ="jump"
                    if nodename =="Struct":
                        leafname ="struct"
                    if nodename =="EmptyStatement":
                        leafname ="none"
                    #leafname =''.join('%s' % v for v in vlist[0])
             #f.write(' ')
             #f.write(leafname) #
    ...
    def edges_tofile(self, parent ='', f = None):
        ...
        if parent !='':
            f.write(parent + '_'+ self.__class__.__name__+ ' ')

        for (child_name, child) in self.children():
            child.edges_tofile(
                parent=my_node_name, f = f)

    def savepathsroot2leaf(self, rootpath='', f=None, branchsepa='|'):

        my_node_name = self.__class__.__name__
        if rootpath=='':
            mypath = self.__class__.__name__
        else:
            mypath = rootpath + ',' + self.__class__.__name__
        ...
        if(len(self.children())==0):
            ...
            for (child_name, child) in self.children():
                child.savepathsroot2leaf(
                    rootpath=mypath, f=f, branchsepa = branchsepa)
...
class ArrayRef(Node):
    ...
    def children(self):
        nodelist = []
        if not (self.name.IsDecl() and self.subscript.IsDecl()):
...
class Assignment(Node):
    ...
    def children(self):
        ...
        if  not (self.lvalue.IsDecl() and self.rvalue.IsDecl()):
...
class BinaryOp(Node):
    ...
    def children(self):
        nodelist = []
        if (self.left is not None) and (not self.left.IsDecl()): nodelist.append(("left", self.left))
        if (self.right is not None) and (not self.left.IsDecl()): nodelist.append(("right", self.right))
        return tuple(nodelist)
...
class DoWhile(Node):
    ...
    def setChildren(self, cond= None, stmt = None):
        self.cond = cond
        self.stmt = stmt
...
class For(Node):
    ...
    def setChildren(self, init = None, cond = None, next = None, stmt = None):
        self.init = init
        self.cond = cond
        self.next = next
        self.stmt = stmt
...
class FuncDef(Node):
    ...
    def children(self):
        nodelist = []
        if self.decl is not None and not self.decl.IsDecl(): nodelist.append(("decl", self.decl))
...
class TernaryOp(Node):
    ...
    def children(self):
        nodelist = []
        if not (self.iftrue.IsDecl() and self.iffalse.IsDecl()):
            if self.cond is not None: nodelist.append(("cond", self.cond))
            if self.iftrue is not None: nodelist.append(("iftrue", self.iftrue))
            if self.iffalse is not None: nodelist.append(("iffalse", self.iffalse))
        return tuple(nodelist)
...
class While(Node):
    __slots__ = ('cond', 'stmt', 'coord', '__weakref__')
    def setChildren(self, cond, stmt):
        self.cond = cond
        self.stmt = stmt
    def __init__(self, cond, stmt, coord=None):
        self.cond = cond
