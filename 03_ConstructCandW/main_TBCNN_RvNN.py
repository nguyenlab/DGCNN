import constructNetWork_TBCNN_RvNN as TC
import cPickle as p
...
def InitByNodes(nodes):
    ...
    if avgdepth < 1:
        avgdepth = 1
    layers = TC.ConstructTreeConvolution(nodes, numFea, numCon, numRecur, numDis, numOut, \
                                         Wleft, Wright, Bconstruct, \
                                         Wcomb_ae, Wcomb_orig, \
                                         Wconv_root, Wconv_left, Wconv_right, Bconv, \
                                         Wrecur_root, Wrecur_left, Wrecur_right, Brecur, \
                                         Wconv_dis, Wrecur_dis, Wout, Bdis, Bout, \
                                         poolCutoff=avgdepth
                                         )
    return layers
